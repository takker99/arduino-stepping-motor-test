...
| `POST` | `/step?steps=N&dir=cw\|ccw` | ステップ実行 |
...
1. Wi-Fi 認証情報をスケッチに差し込み (`SSID`, `PASS` を `secrets.h` 等に分離)
2. 実機を USB 接続して `pio run -t upload` で書き込み
3. シリアルモニタで IP を確認、`curl http://<IP>/status` で疎通確認
4. `POST /step` でモーター回転確認 (要: ブレッドボード + 9V 電源 + ULN2003 + 28BYJ-48 配線済み)
5. 動作結果を [[log]] にフィードバック
6. 将来: AP フォールバック / `/stop` のノンブロッキング化 / 認証

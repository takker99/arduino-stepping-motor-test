...
現状: Wi-Fi API サーバ疎通済み (192.168.11.3) / 5V 供給パス確認済み (E↔赤線 = 5V)。
モーター無通電の原因調査中 (最新の残課題は [[log#2026-08-12 中座]] と
[[log#2026-08-13 note | ULN2003 は DIP 化基板 + 5V 供給パス確認完了]] 参照)。

1. コイル抵抗チェック (電源 OFF, Ω レンジ): 赤↔各相 ≈ 50Ω / 青↔オレンジ ≈ 100Ω / 青↔ピンク = ∞
2. ULN2003 pin 9 (COM) の +5V 接続確認 (DIP 手配線のため)
3. 通電確認: `POST /step?steps=2048&dir=cw` でモーター回転
4. 動作結果を [[log]] にフィードバック
5. 将来: AP フォールバック / `/stop` のノンブロッキング化 / 認証
